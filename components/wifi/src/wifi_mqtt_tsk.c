...
static void WIFI_vMqtt_Event_Handler_CB(esp_mqtt_event_handle_t event)
{
    ...
    switch (event->event_id)
    {
        ...
        case MQTT_EVENT_DATA:
        	...
        	if(sscanf(event->data, CONFIG_WIFI_MQTT_DEFAULT_SUBSCRIBE_FORMAT, &uiAddr, &uiRegAddr, &uiFlag, &uiData) == 4)
        	{
        		   ...
        		   || mMyQueueData.tMyQueueCommand.tMyQueueCommandType == WriteReg)
        		{
        			...
        			xQueueSend(mWifiQueueRec, (void * )&mMyQueueData, (TickType_t)0);
        		}
        	}
            break;
...
void Wifi_Mqtt_vInit(void)
{
	...
	while(1)
	{
		...
		if(mWifiConnected)
		{
			ESP_LOGW(TAG, "------------%d------", mWifiConnected);
			mMqttPublishDataCount = 0;
			mMqttPublishData = malloc(sizeof(MqttPublishDataTPDF) * CONFIG_WIFI_MQTT_DEFAULT_PUBLISH_MAX);
				...
				if(mDevice[i].tDeviceConfig.usConvertAddr != 0)
				{
					for(j = 0; j < CONFIG_DEVICE_MAXSURPPORT_PARTITION_TABLES; j++)
					{
						if(mDevice[i].tDevicePartitionTable[j].usLen)
						{
							...
							{
								mMqttPublishData[mMqttPublishDataCount].cFromat = malloc(sizeof(CONFIG_WIFI_MQTT_DEFAULT_PUBLISH_FORMAT));
								memcpy(mMqttPublishData[mMqttPublishDataCount].cFromat,
										CONFIG_WIFI_MQTT_DEFAULT_PUBLISH_FORMAT,
										sizeof(CONFIG_WIFI_MQTT_DEFAULT_PUBLISH_FORMAT));
			...
			esp_mqtt_client_config_t mqtt_cfg = {
				.host = CONFIG_WIFI_MQTT_DEFAULT_HOST,
				.port = CONFIG_WIFI_MQTT_DEFAULT_PORT,
				.username = CONFIG_WIFI_MQTT_DEFAULT_PRODUCT_ID,
			...
			esp_mqtt_client_register_event(mClient, ESP_EVENT_ANY_ID, WIFI_vMqtt_Event_Handler, mClient);
